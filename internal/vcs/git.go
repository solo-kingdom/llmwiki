...
// wikiMaintenancePaths are regenerated by post-apply maintenance and must be
// committed before git merge to avoid blocking the working tree.
var wikiMaintenancePaths = []string{"wiki/index.md", "wiki/log.md"}

// HasUncommittedWikiMaintenance reports whether wiki/index.md or wiki/log.md
// have unstaged or uncommitted changes in the working tree.
func (r *GitRepo) HasUncommittedWikiMaintenance() (bool, error) {
	out, err := r.gitOutput(append([]string{"status", "--porcelain", "--"}, wikiMaintenancePaths...)...)
	if err != nil {
		return false, fmt.Errorf("git status wiki maintenance: %w", err)
	}
	return strings.TrimSpace(out) != "", nil
}

// CommitWikiMaintenance stages wiki/index.md and wiki/log.md when changed and
// commits them with the given message. Returns empty SHA if nothing to commit.
func (r *GitRepo) CommitWikiMaintenance(message string) (string, error) {
	for _, rel := range wikiMaintenancePaths {
		...
		if err := r.runGit("add", rel); err != nil {
	...
	if err != nil {
		return "", fmt.Errorf("check staged wiki maintenance: %w", err)
	}
	if !hasChanges {
		return "", nil
	}
	...
	if err := r.runGit("commit", "-m", message); err != nil {
		return "", fmt.Errorf("git commit wiki maintenance: %w", err)
	}
