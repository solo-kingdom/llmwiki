...
When an ingest job is created, the system SHALL record a `rules_hash` in job metadata derived from `purpose.md`, `rules.md`, `rules_supplement`, and canonical `prompts.yaml` content.

#### Scenario: Hash recorded on enqueue
- **WHEN** a new ingest job is enqueued
- **THEN** job metadata SHALL include `rules_hash` as a hex digest
